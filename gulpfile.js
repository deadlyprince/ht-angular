...
const config = {
  libraryName: 'ht-angular',
  unscopedLibraryName: 'ht-angular',
  // allSrc: 'src/index.ts',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
  // allTs: 'src/index.ts',
...
gulp.task('build', ['clean'], (cb) => {
  runSequence('compile', 'npm-package', 'rollup-bundle', 'styles', 'themes', cb);
  // runSequence('compile', 'npm-package', 'rollup-bundle', 'styles', 'themes', cb);
});
...
gulp.task('npm-package', (cb) => {
  ...
  let targetPkgJson = {};
  let fieldsToCopy = ['version', 'peerDependencies', 'description', 'keywords', 'author', 'repository', 'license', 'bugs', 'homepage'];
